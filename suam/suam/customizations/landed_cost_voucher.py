...
class LandedCostVoucher(Document):
	...
	if TYPE_CHECKING:
		...
		company: DF.Link
		distribute_charges_based_on: DF.Literal["Qty", "Amount", "Distribute Manually", "Weight"]
		...
		for pr in self.get("purchase_receipts"):
			if pr.receipt_document_type and pr.receipt_document:
				...
				for d in pr_items:
					...
					item.custom_weight = d.total_weight
					item.qty = d.qty
	...
	def set_applicable_charges_on_item(self):
		if self.get("taxes") and self.distribute_charges_based_on != "Distribute Manually":
			# Map user-friendly value to actual item field
			field_map = {
				"amount": "amount",
				"qty": "qty",
			...
			based_on_key = self.distribute_charges_based_on.strip().lower()
			based_on_field = field_map.get(based_on_key)

			if not based_on_field:
				frappe.throw(_("Invalid 'Distribute Charges Based On' option: {0}").format(self.distribute_charges_based_on))

			total_item_cost = 0.0
			total_charges = 0.0
			item_count = 0

			# Calculate total value for chosen basis
			for item in self.get("items"):
			...
			# Adjust last item to fix rounding difference
	...
	def validate_applicable_charges_for_item(self):
		...
		# Map friendly name to actual field
		field_map = {
			"amount": "amount",
			"qty": "qty",
			"weight": "custom_weight"
		...
		distribute_based_on = self.distribute_charges_based_on.strip().lower()
		based_on = field_map.get(distribute_based_on)

		if not based_on and distribute_based_on != "distribute manually":
			frappe.throw(_("Unsupported 'Distribute Charges Based On': {0}").format(self.distribute_charges_based_on))

		if distribute_based_on != "distribute manually":
